...
const credenzialiPath = "/Users/its/Documents/Backend/express/Samuele_Ceci/progetto_books/credenziali.json";
...
function leggiCredenziali() {
    try {
        const data = fs.readFileSync(credenzialiPath);
        return JSON.parse(data);
    } catch (error) {
        console.error("Errore nella lettura del file delle credenziali:", error);
        return [];
    }
}

function scriviCredenziali(credenziali) {
    try {
        fs.writeFileSync(credenzialiPath, JSON.stringify(credenziali, null, 2));
    } catch (error) {
        console.error("Errore nella scrittura del file delle credenziali:", error);
    }
}
...
app.put('/credenziali', (req, res) => {
    const nuoveCredenziali = req.body;
    scriviCredenziali(nuoveCredenziali);
    res.status(200).json({ message: 'Credenziali aggiornate con successo' });
});
